...
typedef enum { MODE_CONTINOUS = 0, MODE_SINGLE_SHOT } OperatingMode_t;
...
typedef enum { COMP_HYSTERESIS = 0, COMP_WINDOW } CompareMode_t;
...
typedef enum { LATCHING_NONE = 0, LATCHING_COMPARATOR } LatchingMode_t;
...
ADS1115_Handle_t *ADS1115_init(I2C_HandleTypeDef *hi2c, uint16_t Addr,
                               ADS1115_Config_t config);

void ADS1115_updateConfig(ADS1115_Handle_t *pConfig, ADS1115_Config_t config);

void ADS1115_updateI2Chandler(ADS1115_Handle_t *pConfig,
...
void ADS1115_setThresholds(ADS1115_Handle_t *pConfig, int16_t lowValue,
                           int16_t highValue);
...
extern ADS1115_Config_t configReg;
extern ADS1115_Handle_t *pADS_1;
