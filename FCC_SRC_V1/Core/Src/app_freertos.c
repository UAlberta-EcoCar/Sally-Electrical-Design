...
uint32_t canReceiveBuffer[1024];
...
const osMessageQueueAttr_t RxHeaderQue_attributes = { .name = "RxHeaderQue",
		.cb_mem = &RxHeaderQueControlBlock, .cb_size =
...
uint8_t RxDataQueBuffer[1024 * sizeof(uint8_t)];
osStaticMessageQDef_t RxDataQueControlBlock;
const osMessageQueueAttr_t RxDataQue_attributes = { .name = "RxDataQue",
...
const osThreadAttr_t usb_attributes = { .name = "usbReceive", .stack_mem =
		&usbBuffer[0], .stack_size = sizeof(usbBuffer), .cb_mem =
		&usbControlBlock, .cb_size = sizeof(usbControlBlock), .priority =
...
void MX_FREERTOS_Init(void) {
	...
	canQueRxHeaderHandle = osMessageQueueNew(512, sizeof(uint32_t),
			&RxHeaderQue_attributes);

	/* creation of RxDataQue */
	canQueRxDataHandle = osMessageQueueNew(1024, sizeof(uint8_t),
			&RxDataQue_attributes);

	/* USER CODE BEGIN RTOS_QUEUES */
	/* add queues, ... */

	/* creation of usbQueReceive */
	usbQueReceiveHandle = osMessageQueueNew(512, sizeof(char),
			&usbQueReceive_attributes);

	/* creation of usbQueSend */
	usbQueSendHandle = osMessageQueueNew(512, sizeof(char),
